...
try:
    from .car_nmpc_model import DEFAULT_URDF_PATH, default_state, describe_model, export_car_ode_model
except ImportError:
    from car_nmpc_model import DEFAULT_URDF_PATH, default_state, describe_model, export_car_ode_model


def _build_state_weight(nx: int, nq: int, nv: int) -> np.ndarray:
    q_weights = np.ones(nq)
    v_weights = np.ones(nv)

    # floating base translation xyz
    q_weights[0:3] = np.array([40.0, 5.0, 80.0])
    # floating base quaternion xyzw
    q_weights[3:7] = np.array([150.0, 1500.0, 150.0, 150.0])
    # left/right continuous wheel joints are stored as cos/sin pairs
    q_weights[7:11] = np.array([0.1, 0.1, 0.1, 0.1])

    # floating base spatial velocity: linear xyz + angular xyz
    v_weights[0:6] = np.array([20.0, 5.0, 40.0, 20.0, 400.0, 20.0])
    # wheel angular velocities
    v_weights[6:8] = np.array([0.5, 0.5])

    return np.diag(np.concatenate([q_weights, v_weights]))
...
def build_ocp(
    ...
    Q = _build_state_weight(nx=nx, nq=nq, nv=nv)
    R = np.diag([0.5, 0.5])
    ...
    x_ref = np.asarray(default_state(urdf_path)).reshape(-1)
    u_ref = np.zeros(nu)
    ocp.cost.yref = np.concatenate([x_ref, u_ref])
    ocp.cost.yref_e = x_ref.copy()

    ocp.constraints.x0 = x_ref.copy()
